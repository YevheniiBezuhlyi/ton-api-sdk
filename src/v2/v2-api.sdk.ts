...
export class TonApiSdkV2 extends Base {
    ...
    async getExtendedAddressInformation(address: string): Promise<V2ApiResponse<V2ExtendedAddressInformation>> {
        return super.request<V2ApiResponse<V2ExtendedAddressInformation>>(
            'GET',
    ...
    async getTokenData(address: string): Promise<V2ApiResponse<V2TokenData>> {
        try {
            const response = await super.request<V2ApiResponse<V2TokenData>>('GET', '/api/v2/getTokenData', {
                address,
            });
            return response;
        } catch (error) {
    ...
    async jsonRpc(params: { method: string; params: any; id: string; jsonrpc: string }): Promise<any> {
